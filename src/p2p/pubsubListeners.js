...
export async function listenForPubKeys() {
    ...
    await s.node.pubsub.subscribe(topicPubKeys, async (msg) => {
...
export const listenForSignatures = async (msg) => {
    if (msg.from.toString() !== s.myId) {
...
// Rätsler listener 
export const rästlerListener = async (msg) => {
    if (msg.from.toString() !== s.myId) {
        if (s.rolle == "rätsler") {
            let data = await msg.data
            let message = uint8ArrayToString(data)
            ...
            if (message.includes('multiSigAddress')) {
                ...
                //await s.node.pin.add(parseJson.keys.path, true)    
                message = message.split(" ")[1]
